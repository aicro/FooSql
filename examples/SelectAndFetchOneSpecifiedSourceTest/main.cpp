...
int main()
{
    ...
    try
    {
        ...
        Row rslt;
        int count = 0;
        while ((char**)(rslt = query_rslt->Fetch(&dbLocation2, success)) != NULL )
        {
            unsigned long * lengths = query_rslt->GetCurrentRowColumnsLength(&dbLocation2, success);

            cout << "id   = " << rslt["id"] << "(" << lengths[0] << ")" << endl;
            cout << "name = " << rslt["name"] << "(" << lengths[1] << ")" << endl;
            cout << "time = " << rslt["uptime"] << "(" << lengths[2] << ")" << endl << endl;
        }
