...
int main()
{
    ...
    try
    {
        ...
        while (row_count++ < 10)
        {
            ...
            insertFilter.AppendColumnValue("id", Value((long long)(row_count)), ignore_cols);

            // there is a "\0" in the string, so we need to give the length
            insertFilter.AppendColumnValue("name", Value((long long)(7), "ab\0defg"), ignore_cols);
            ...
            insertFilter.AppendColumnValue("uptime", Value("NOW()", true, false ), ignore_cols);
